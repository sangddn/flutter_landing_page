...
/// A [Gap] that scales with [MediaQuery.textScalerOf].
///
class ScaledGap extends StatelessWidget {
  const ScaledGap(
    this.mainAxisExtent, {
  ...
  Widget build(BuildContext context) {
    ...
    return Gap(
      scaler.scale(mainAxisExtent),
      crossAxisExtent: crossAxisExtent?.let((x) => scaler.scale(x)),
      color: color,
    );
  }
}
