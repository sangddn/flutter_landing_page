...
mixin _RHooks on ChangeNotifier {
  RSingleState<T> useState<T>(T value) =>
      RSingleState<T>(safeNotifyListeners, value);
}

abstract class RChangeNotifier extends ChangeNotifier with _RHooks {
  @mustCallSuper
  void init() {}
}

class RSingleState<R> {
  RSingleState(
    this._notify,
    this._value,
  );
...
extension SafeNotifier on ChangeNotifier {
